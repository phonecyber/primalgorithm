...
void start(int &menu, int &type, int &example, int &numNode)
{
	...
	cout << "2. Undirected graph" << endl << endl;
...
public:
	...
	bool findInVisitedMST(char el)
	{
		for (list<Edges>::iterator it = visitedMST.begin(); it != visitedMST.end(); it++)
		{
			if (it->edge->data == el)
			{
				return true;
	...
	void MSTPrim()
	{
		char el = 'A'; // Actually, it doesn't matter with the start node ..so set it to 'A' (or 'B', 'C', ...)
		cout << "\n-------------------------------\n\n";
		cout << "#### Prim's Algorithm\n\n";

		visitedMST.push_back(Edges(0)); // visit start node first
		visitedMST.back().source = arrPtr[0];
		visitedMST.back().edge = arrPtr[0];
		...
		for (int count = 0; count < numNode - 1; count++)
		{
			for (list<Edges>::iterator it = visitedMST.begin(); it != visitedMST.end(); it++)
			{
				i = index_path(it->edge->data);
				for (int j = 0; j < numNode; j++)
				{
					if (!findInVisitedMST(arrPtr[j]->data) && (matrix[i][j] != 0))
					{
						// find min connected cost
						if (matrix[i][j] < min)
						{
							min = matrix[i][j];
							indexSrc = i;
			...
			visitedMST.push_back(Edges(min));
			visitedMST.back().source = arrPtr[indexSrc];
			visitedMST.back().edge = arrPtr[indexDest];
			min = INFINITY;
		}

		visitedMST.pop_front();
		for (list<Edges>::iterator it = visitedMST.begin(); it != visitedMST.end(); it++)
		{
			cout << "\t" << it->source->data << " --> " << it->edge->data << " = " << it->weight << endl;
		}
	}
